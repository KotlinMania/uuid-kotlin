# Claude Code Project Instructions — uuid-kotlin

## Project Overview

This is **uuid-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`uuid`](https://crates.io/crates/uuid).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/uuid/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/uuid>

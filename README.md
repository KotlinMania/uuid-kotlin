...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`uuid`](https://crates.io/crates/uuid).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `uuid` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:uuid-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `uuid` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
